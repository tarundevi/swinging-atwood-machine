...
pendulum = sphere(pos = rope1.axis, radius = 1.5, color = color.red, make_trail = True, trail_color = color.black)
block = sphere(pos = rope2.axis, radius = 2, color = color.red )

extrusion( path = [ vec(0,0,0.3) , vec(0,0,-0.3)], shape = shapes.circle(radius=1) , color= vec(.7,.7,.7) )
extrusion( path = [ vec(-d,0,0.3) , vec(-d,0,-0.3)], shape = shapes.circle(radius=1) , color= vec(.7,.7,.7) )
